...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-sm-4">
		<h2>การสั่งซื้อสินค้า Pre-Order</h2><br>
		<ol class="breadcrumb">
			<li>
				<a href="{{ url('admin') }}">หน้าแรก</a>
			</li>
			<li class="active">
				<strong>การสั่งซื้อสินค้า Pre-Order</strong>
			</li>
		</ol>
	</div>
</div>
				...
				<div class="ibox-content">
					<table class="table table-striped table-bordered table-hover " id="products" >
						...
						<tbody>
							...
							<tr>
								<td>{{ $i++ }}</td>
								<td>{{ $order->order_number }} <a href="{{ url('admin/pre-order/detail/'.$order->id) }}"><i class="fa fa-external-link"> </i></a></td>
								...
								<td>
									@if(!$order->send_to_farmer)
										<a href="{{ url('admin/pre-order/sendToFarm/'.$order->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-sign-in"> </i> ส่งรายการสั่งซื้อให้ฟาร์ม</a>
									@else
								...
								<td>
									@if($order->order_status == 'paid')
										<a href="{{ url('admin/pre-order/activeShipped/'.$order->id) }}" class="btn btn-warning btn-sm"><i class="fa fa-truck"> </i>   ยืนยันการจัดส่ง</a>
									@endif
