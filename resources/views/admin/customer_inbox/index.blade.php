...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-sm-4">
		<h2>ข้อความลูกค้า</h2><br>
		<ol class="breadcrumb">
			<li>
				<a href="{{ url('admin') }}">หน้าแรก</a>
			</li>
			<li class="active">
				<strong>ข้อความลูกค้า</strong>
			</li>
		</ol>
	</div>
</div>
...
<div class="mail-box-header">
    ...
    <tr class="{{ $inbox->read ? 'read' : 'unread' }}">
        ...
        <td class="mail-ontact"><a href="{{ url('admin/inbox/customer/detail/'.$inbox->slug) }}">
            @if($inbox->admin)
                <i class="fa fa-reply"></i> {{ $inbox->customer->firstname }} {{ $inbox->customer->lastname }}
            @else
                {{ $inbox->customer->firstname }} {{ $inbox->customer->lastname }}
            @endif
            </a></td>
        <td class="mail-subject"><a href="{{ url('admin/inbox/customer/detail/'.$inbox->slug) }}">{{ $inbox->topic }}</a></td>
